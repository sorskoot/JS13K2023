import {CAP, MAT_STATE, Material, RENDER_ORDER, stateToBlendFunc} from './material.js';
...
export class RenderPrimitive {
    ...
    waitForComplete(): Promise<RenderPrimitive> {
        if (!this._promise) {
            if (!this._material) {
...
export class Renderer {
    ...
    _programCache: {};
    _textureCache: {};
    _renderPrimitives: any[];
    _cameraPositions: never[];
    _vaoExt: any;
    _defaultFragPrecision: string;
    _depthMaskNeedsReset: boolean;
    _colorMaskNeedsReset: boolean;
    _globalLightColor: any;
    _globalLightDir: any;
    _multiview: any;
    constructor(gl) {
            ...
            if (program != primitive._material._program) {
                ...
                if (views.length == 1) {
                    if (!this._multiview) {
                        ...
                        gl.uniformMatrix4fv(
                            program.uniform.LEFT_PROJECTION_MATRIX,
                            false,
                            views[0].projectionMatrix
                        );
                        gl.uniformMatrix4fv(
                            program.uniform.LEFT_VIEW_MATRIX,
                            false,
                            views[0].viewMatrix
                        );
                        gl.uniformMatrix4fv(
                            program.uniform.RIGHT_PROJECTION_MATRIX,
                            false,
                            views[0].projectionMatrix
                        );
                        gl.uniformMatrix4fv(
                            program.uniform.RIGHT_VIEW_MATRIX,
                            false,
                            views[0].viewMatrix
                        );
                        gl.uniform3fv(
    ...
    _getMaterialProgram(material, renderPrimitive) {
        ...
        if (key in this._programCache) {
            ...
            fullVertexSource += VERTEX_SHADER_SINGLE_ENTRY;
