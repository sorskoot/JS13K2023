...
export class WebGLRenderer {
    ...
    private _updateSampler(sampler: Sampler): WebGLSampler {
        ...
        if (sampler.needsUpdate) {
            if (sampler.anisotropy) {
                const anisotropyExt = this.gl.getExtension(
            ...
            this.gl.samplerParameteri(
                ...
                GL_FILTERS[sampler.magFilter]
            );
            this.gl.samplerParameteri(
                target,
                GL_TEXTURE_MIN_FILTER,
                GL_FILTERS[sampler.minFilter]
            );

            this.gl.samplerParameteri(
                target,
                GL_TEXTURE_WRAP_S,
                GL_WRAPPINGS[sampler.wrapS]
            );
            this.gl.samplerParameteri(
                target,
                GL_TEXTURE_WRAP_T,
                GL_WRAPPINGS[sampler.wrapT]
            );
    ...
    setRenderTarget(renderTarget: RenderTarget | null): void {
        ...
        if (!FBO || renderTarget.needsUpdate) {
            ...
            for (const texture of renderTarget.textures) {
                ...
                const target = this._updateTexture(
                    texture,
                    renderTarget.width,
                    renderTarget.height
                );
    ...
    compile(mesh: Mesh, camera?: Camera): WebGLCompiled {
        ...
        if (!program) {
            ...
            this.gl.shaderSource(
                vertexShader,
                mesh.material.vertex ?? mesh.material.compute
            );
            ...
            this.gl.shaderSource(
                fragmentShader,
                mesh.material.fragment ?? '#version 300 es\nvoid main(){}'
            );
        ...
        for (const key in mesh.material.uniforms)
            this._setUniform(program, key, mesh.material.uniforms[key]);
    ...
    sort(scene: Object3D, camera?: Camera): Mesh[] {
        ...
        return renderList.sort(
            (a, b) =>
                ...
                (!!camera &&
                    this._v
                        .set(b.matrix[12], b.matrix[13], b.matrix[14])
                        .applyMatrix4(camera.projectionViewMatrix).z -
                        this._v
                            .set(a.matrix[12], a.matrix[13], a.matrix[14])
                            .applyMatrix4(camera.projectionViewMatrix).z) ||
        ...
        for (const node of renderList) {
            ...
            if (index)
                ...
                this.gl.drawArraysInstanced(
                    mode,
                    start,
                    min(count, position.data.length / position.size),
                    node.instances
                );
    ...
    compute(node: Mesh): void {
        ...
        const transformFeedback = (this._transformFeedback ??=
            this.gl.createTransformFeedback()!);
